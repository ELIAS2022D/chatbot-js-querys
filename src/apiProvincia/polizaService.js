 ...
 * @param {Object} datos
 * @param {string} datos.poliza
 * @returns {Promise<Buffer>} PDF de la póliza si se encuentra.
 */


export const obtenerPolizaPdf = async (datos) => {
  ...
  console.log(`🔄 Buscando productor correcto para póliza ${datos.poliza}...\n`);
  ...
  for (const productorId of productores) {
    ...
    try {
      ...
      const url = `${API_URL}/obtenerPdf/${sucursal}/${ramo}/${datos.poliza}/${endoso}?apikey=${API_KEY}`;
